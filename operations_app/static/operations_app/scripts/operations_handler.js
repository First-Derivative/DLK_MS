...
function addOperations(new_operations) {
  ...
  for (const field of Object.entries(new_operations)) {
    if(new_operations[field[0]] === null){alerted = true}
  }

  alerted_tag = `<img src="${alertedHD_src}" width="32" height="32" class="hoverable" id="card-edit-${new_operations.project_code}" style="padding-bottom: 0.2em;margin-right: 0.5em" name="${new_operations.project_code}" alt="Needs Entry">`
  ...
  operations_card_template =
    ...
    <div class="card-body d-flex justify-content-between" id="card-body-${new_operations.project_code}">
      ...
      <div class="card_row">
        <p class="card-text" id="status_${new_operations.project_code}"><span class="text-muted">Status: </span>${new_operations.status}</p>
